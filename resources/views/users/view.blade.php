...
<div class="bg-white shadow rounded-lg p-6 mb-10 mt-6">
    ...
    <div class="grid grid-cols-2 gap-5">
        ...
        <div>
            <p class="mb-2"><strong>Created At:</strong> {{ $user->created_at->format('Y-m-d H:i:s') }}</p>
            <p class="mb-2"><strong>Updated At:</strong> {{ $user->updated_at->format('Y-m-d H:i:s') }}</p>
        </div>
    </div>
</div>

<div class="flex justify-between items-center mb-6">
    <a href="{{ route('tasks.create', ['user_id' => $user->id]) }}" class="bg-green-500 text-white px-4 py-2 rounded inline-block">
        Add New Task
    </a>
...
<div class="bg-white shadow rounded-lg p-6">
    ...
    @else
    <table class="table-auto w-full border border-gray-700 mt-4">
        <thead>
